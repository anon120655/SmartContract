...
namespace SmartContract.Infrastructure.Repositorys.ContractTypeVendor.TMaster
{
    public class T11Repo : IT11Repo
    {
        ...
        public T11Repo(IRepositoryWrapper repo, IRepositoryBase db, IMapper mapper, IWebHostEnvironment env, IOptions<AppSettings> settings)
        {
            _repo = repo;
            _db = db;
            _env = env;
        ...
        public IEnumerable<ContractStationDTO> GetList(SearchOptionCTV Condition = null)
        {
            ...
            var contractStations = _repo.Context.ContractStations.Where(x => x.Used && x.ContractStyleCode == "S41" && contractTypesUse.Contains(x.ContractTypeId)).OrderByDescending(x => x.EditDate);
        ...
        public async Task DeleteAsync(TAllMasterVendorView indata)
        {
            String idSmctMaster = indata.ParameterCondition.IdSmctMaster;
            using (var _transaction = _repo.BeginTransaction())
            {
                await _repo.ContractShare.UpdateStatusContract(new ParameterStatusStation()
                {
                    ...
                    Status = ContractStatusAll.S0Cancel,
                    StationStatusPrev = ContractStationStatusAll.S1Draft,
                    StationStatusCurr = ContractStationStatusAll.S8Cancel,
                    ItemStatusPrev = ContractStationStatusItemAll.S11Draft,
                    ItemStatusCurr = ContractStationStatusItemAll.S81CancelVendor
                });
                await _transaction.CommitAsync();
            }
        }
