...
namespace SmartContract.Infrastructure.Repositorys.ContractTypeVendor.TMaster
{
    public class T08Repo : IT08Repo
    {
        ...
        public T08Repo(IRepositoryWrapper repo, IRepositoryBase db, IMapper mapper, IWebHostEnvironment env, IOptions<AppSettings> settings)
        {
            _repo = repo;
            _db = db;
            _env = env;
        ...
        public IEnumerable<ContractStationDTO> GetList(SearchOptionCTV Condition = null)
        {
            ...
            var contractStations = _repo.Context.ContractStations.Where(x => x.Used && x.ContractStyleCode == "S2" && x.DepartmentCode == departmentCode).OrderByDescending(x => x.EditDate);
