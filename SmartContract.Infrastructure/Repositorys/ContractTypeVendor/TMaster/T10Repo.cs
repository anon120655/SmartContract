...
namespace SmartContract.Infrastructure.Repositorys.ContractTypeVendor.TMaster
{
    public class T10Repo : IT10Repo
    {
        ...
        public T10Repo(IRepositoryWrapper repo, IRepositoryBase db, IMapper mapper, IWebHostEnvironment env, IOptions<AppSettings> settings)
        {
            _repo = repo;
            _db = db;
            _env = env;
        ...
        public IEnumerable<ContractStationDTO> GetList(SearchOptionCTV Condition = null)
        {
            ...
            var contractStations = _repo.Context.ContractStations.Where(x => x.Used && x.ContractStyleCode == "S3" && x.DepartmentCode == departmentCode && contractTypesUse.Contains(x.ContractTypeId)).OrderByDescending(x => x.EditDate);
