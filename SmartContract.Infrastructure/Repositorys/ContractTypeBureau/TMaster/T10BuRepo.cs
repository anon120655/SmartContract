...
{
    public class T10BuRepo : IT10BuRepo
    {
        ...
        public T10BuRepo(IRepositoryWrapper repo, IRepositoryBase db, IMapper mapper, IWebHostEnvironment env, IOptions<AppSettings> settings)
        {
            _repo = repo;
            _db = db;
            _env = env;
        ...
        public async Task<TAllMasterVendorView> InitialData(TAllMasterVendorView indata)
        {
            ...
            indata.CTVendor.GetLookUp = new LookUpResource()
            {
                ...
                BudgetcodeList = await _repo.MasterData.BudgetcodeList(),
                CommitteeList = await _repo.MasterData.CommitteeList(),
                NhsoSigner = await _repo.MasterData.NhsoSigner(departmentCode),
                VNhsoEmployeeAll = await _repo.MasterData.VNhsoEmployeeAll(departmentCode)
            };
        ...
        private async Task<TAllMasterVendorView> ViewContractDetail(TAllMasterVendorView indata)
        {
            ...
            var contractDetail = await _repo.Context.ContractDetail10s.Where(x => x.IdContract == _IdContract && x.Used).FirstOrDefaultAsync();
            if (contractDetail != null)
            {
                indata.InfoConDetail.ContractDetail10 = _mapper.Map<ContractDetail10DTO>(contractDetail);
            }
        ...
        private async Task UpdateContractDetail(TAllMasterVendorView indata)
        {
            ...
            if (_Contract != null)
            {
                var _ContractDetail = indata.InfoConDetail.ContractDetail10;
                var contractDetail = await _repo.Context.ContractDetail10s.Where(x => x.IdContract == _Contract.IdContract && x.Used).FirstOrDefaultAsync();
                if (contractDetail != null)
                {
                ...
                {
                    contractDetail = _mapper.Map<ContractDetail10>(_ContractDetail);
