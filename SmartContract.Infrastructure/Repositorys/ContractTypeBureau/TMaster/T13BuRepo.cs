...
{
    public class T13BuRepo : IT13BuRepo
    {
        ...
        public T13BuRepo(IRepositoryWrapper repo, IRepositoryBase db, IMapper mapper, IWebHostEnvironment env, IOptions<AppSettings> settings)
        {
            _repo = repo;
            _db = db;
            _env = env;
        ...
        private async Task<TAllMasterVendorView> ViewContractDetail(TAllMasterVendorView indata)
        {
            ...
            var contractDetail = await _repo.Context.ContractDetail13s.Where(x => x.IdContract == _IdContract && x.Used).FirstOrDefaultAsync();
            if (contractDetail != null)
            {
                indata.InfoConDetail.ContractDetail13 = _mapper.Map<ContractDetail13DTO>(contractDetail);
                indata.InfoConDetail.ContractDetail13.IsP1 = contractDetail.P1 == "1";
                indata.InfoConDetail.ContractDetail13.IsP6 = contractDetail.P6 == "1";
            }
        ...
        private async Task UpdateContractDetail(TAllMasterVendorView indata)
        {
            ...
            if (_Contract != null)
            {
                var _ContractDetail = indata.InfoConDetail.ContractDetail13;
                var contractDetail = await _repo.Context.ContractDetail13s.Where(x => x.IdContract == _Contract.IdContract && x.Used).FirstOrDefaultAsync();
                if (contractDetail != null)
                {
                    _mapper.Map(_ContractDetail, contractDetail);

                    //contractDetail.P1 = _ContractDetail.IsP1 ? "1" : "2";
                    //contractDetail.P6 = _ContractDetail.IsP6 ? "1" : "2";
                ...
                {
                    contractDetail = _mapper.Map<ContractDetail13>(_ContractDetail);
