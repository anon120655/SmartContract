...
{
    public class T09BuRepo : IT09BuRepo
    {
        ...
        public T09BuRepo(IRepositoryWrapper repo, IRepositoryBase db, IMapper mapper, IWebHostEnvironment env, IOptions<AppSettings> settings)
        {
            _repo = repo;
            _db = db;
            _env = env;
        ...
        private async Task<TAllMasterVendorView> ViewContractDetail(TAllMasterVendorView indata)
        {
            ...
            var contractDetail = await _repo.Context.ContractDetail09s.Where(x => x.IdContract == _IdContract && x.Used).FirstOrDefaultAsync();
            if (contractDetail != null)
            {
                indata.InfoConDetail.ContractDetail09 = _mapper.Map<ContractDetail09DTO>(contractDetail);
            }
        ...
        private async Task UpdateContractDetail(TAllMasterVendorView indata)
        {
            ...
            if (_Contract != null)
            {
                var _ContractDetail = indata.InfoConDetail.ContractDetail09;
                var contractDetail = await _repo.Context.ContractDetail09s.Where(x => x.IdContract == _Contract.IdContract && x.Used).FirstOrDefaultAsync();
                if (contractDetail != null)
                {
                ...
                {
                    contractDetail = _mapper.Map<ContractDetail09>(_ContractDetail);
