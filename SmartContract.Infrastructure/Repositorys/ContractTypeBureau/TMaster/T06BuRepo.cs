...
{
    public class T06BuRepo : IT06BuRepo
    {
        ...
        public T06BuRepo(IRepositoryWrapper repo, IRepositoryBase db, IMapper mapper, IWebHostEnvironment env, IOptions<AppSettings> settings)
        {
            _repo = repo;
            _db = db;
            _env = env;
        ...
        public async Task<TAllMasterVendorView> InitialData(TAllMasterVendorView indata)
        {
            ...
            indata.CTVendor.GetLookUp = new LookUpResource()
            {
                ...
                VDFiBankVendorList = await _repo.MasterData.VDFiBankVendorList("12464"/*VendorId*/)
            };
        ...
        private async Task<TAllMasterVendorView> ViewContractDetail(TAllMasterVendorView indata)
        {
            ...
            var contractDetail = await _repo.Context.ContractDetail06s.Where(x => x.IdContract == _IdContract && x.Used).FirstOrDefaultAsync();
            if (contractDetail != null)
            {
                indata.InfoConDetail.ContractDetail06 = _mapper.Map<ContractDetail06DTO>(contractDetail);
            }
        ...
        private async Task UpdateContractDetail(TAllMasterVendorView indata)
        {
            ...
            if (_Contract != null)
            {
                var _ContractDetail = indata.InfoConDetail.ContractDetail06;
                var contractDetail = await _repo.Context.ContractDetail06s.Where(x => x.IdContract == _Contract.IdContract && x.Used).FirstOrDefaultAsync();
                if (contractDetail != null)
                {
                ...
                {
                    contractDetail = _mapper.Map<ContractDetail06>(_ContractDetail);
