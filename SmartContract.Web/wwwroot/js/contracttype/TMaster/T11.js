...
function InitialT11() {
    //ให้ระบบคำนวนวงเงินให้เลย โดย User ไม่ต้องระบุ คือ  วงเงินตั้งต้นสัญญา * 0.5
    var InfoContract_Budget = parseFloat($('#InfoContract_Budget').val());
    if (!isNaN(InfoContract_Budget)) {
        //console.log(InfoContract_Budget)
        var _DeductedAmountMoney = parseFloat($('#InfoGuaranteeContract_DeductedAmountMoney').val());
        //console.log(_DeductedAmountMoney)
        if (!isNaN(_DeductedAmountMoney) && _DeductedAmountMoney == 0) {
            $('#InfoGuaranteeContract_DeductedAmountMoney').val(InfoContract_Budget * 0.5);
        }
    }
}
...
completedForm = function (xhr) {
    ...
    if (xhr.responseJSON.Status == false && xhr.responseJSON.MessagError != null) {
        ...
        if (xhr.responseJSON.Result != undefined) {
            ...
            console.log(ApprovalReqStation)
